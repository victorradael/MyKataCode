...
// Charlie doesn't like these sessions, since the motive usually repeats. He isn't
...
// lst at most N times without reordering. For example if N = 2, and the input
...
function deleteNth(arr,n){
  ...
  var listOfOcurrences = {};

  const deletedOcurrences = arr.filter(number => {
    listOfOcurrences[number] === undefined ? listOfOcurrences[number] = 1 : listOfOcurrences[number] += 1;
    if(listOfOcurrences[number] <= n) {
      return number;
    }
  });
  
  return  deletedOcurrences
